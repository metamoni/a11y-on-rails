RSpec.describe 'Visit Validations page', type: :feature do
  ...
  it 'displays the correct headings', :aggregate_failures do
    ...
    expect(page.all('h2')[2]).to have_text 'Validation with HTML'
    expect(page.all('h2')[3]).to have_text 'Try it yourself'
    expect(page.all('h2')[4]).to have_text 'Custom form validation'
    expect(page.all('h2')[5]).to have_text 'Resources'
  end
